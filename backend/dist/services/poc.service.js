...
class PocService {
     ...
     */
    static aggregateActualCost(projectId) {
        let labor = 0;
        let materials = 0;
        ...
        if (!project)
            return { total: 0, breakdown: { labor: 0, materials: 0, subcontractors: 0, other: 0 } };
        // 1. Labor Cost Integration (Real-time from payroll)
        if (projectId === 'p1') {
            labor = mock_data_1.laborStats.financials.actualCost * 0.6;
        }
        else if (projectId === 'p2') {
            labor = mock_data_1.laborStats.financials.actualCost * 0.4;
        }
        // 2. Subcontractor Bills Integration (Approved and Paid Invoices)
        mock_data_1.vendors.forEach(v => {
     ...
     */
    static runConsolidation(projectId) {
        const project = mock_data_1.projects.find(p => p.id === projectId);
        if (!project || !project.pocDetails)
            return null;
