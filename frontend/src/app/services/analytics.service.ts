...
export class AnalyticsService {
  ...
  refreshAll() {
    ...
    this.http.get<ContractorMetric[]>(`${environment.apiUrl}/contractor-metrics`).subscribe({
      next: (c) => this.contractorsSubject.next(c),
      error: () => this.populateMocks() // Fallback to mocks if server not updated
    });
    
    this.http.get<LaborStats>(`${environment.apiUrl}/labor-stats`).subscribe(s => this.laborStatsSubject.next(s));
    this.http.get<ReportVaultItem[]>(`${environment.apiUrl}/reports`).subscribe(v => this.reportVaultSubject.next(v));
    this.http.get<SiteUpdate[]>(`${environment.apiUrl}/updates`).subscribe(u => this.siteUpdatesSubject.next(u));
  }
